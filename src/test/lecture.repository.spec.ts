...
describe('LectureRepository', () => {
  ...
  describe('동시성 테스트', () => {
    it('여러 요청이 동시에 들어올 때 정확히 30명만 등록되어야 함', async () => {
      ...
      const promises = Array(40).fill(null).map(() => lectureRepository.incrementConcurrency(1));
      ...
      expect(mockLectureRepo.findOne).toHaveBeenCalledTimes(40);
      expect(mockLectureRepo.increment).toHaveBeenCalledTimes(40);
    });
  });
