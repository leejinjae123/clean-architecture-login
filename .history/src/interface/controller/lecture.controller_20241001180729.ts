...
export class LectureController {
  ...
  async applyForLecture(@Body() body: { userId: string; lectureId: number }) {
    return this.LectureUseCase.execute(body.userId, body.lectureId);
  }

  @Get('available')
  async getAvailableLectures(@Query('date') date: string) {
    return this.LectureUseCase.execute(new Date(date));
  }

  @Get('user/:userId')
  async getUserLectures(@Param('userId') userId: string) {
    return this.LectureUseCase.execute(userId);
  }
}