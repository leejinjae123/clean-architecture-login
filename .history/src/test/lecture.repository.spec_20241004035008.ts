...
describe('LectureRepository', () => {
  ...
  describe('동시성 테스트', () => {
    it('여러 요청이 동시에 들어올 때 정확히 30명만 등록되어야 함', async () => {
      ...
      mockLectureRepo.findOne.mockImplementation(() => {
        return Promise.resolve({ ...lecture, currentAttendees: callCount });
      });
  
      mockLectureRepo.save.mockImplementation((savedLecture: Lecture) => {
        if (callCount < 30) {
          callCount++;
          return Promise.resolve({ ...savedLecture, currentAttendees: callCount });
        }
  ...
  describe('정확히 30명 제한 테스트', () => {
    it('29명일 때는 등록 성공, 30명일 때는 등록 성공, 31명째는 등록 실패해야 함', async () => {
        ...
        (redLock.acquire as jest.Mock).mockImplementation(() => Promise.resolve({
          release: jest.fn().mockResolvedValue(true),
        }));
